...
### v4：Agent-Per-Phase 架构

每个流水线阶段由独立 Agent 执行，拥有独立上下文窗口。主进程是轻量编排器——只负责派发 Agent、检查文件存在、读取结论行。消除上下文累积，提升可靠性。

Skill 文件是领域逻辑的**单一真相源**。Agent Prompt（`.specify/templates/agent-prompts/`）是薄壳，引用 Skill 获取规则，只补充 I/O 路径和执行约束。
...
└── .specify/
    ...
    ├── specs/               功能规格目录
    ├── templates/
    │   ├── agent-prompts/   Agent 薄壳模板（10 个文件）
    │   └── *.md             文档模板
    └── scripts/             工具脚本
```
