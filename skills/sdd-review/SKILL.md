...
> **必须在向用户汇报完成后立即执行，不可跳过。**

评审报告输出完成且已向用户汇报后，**立即执行**以下通知发送：

1. 检查 `.specify/notification.json` 是否存在
2. 如果存在且 `enabled` 不为 false，**立即调用** `/sdd-notify review-complete {feature_id}`
...
14. **通知必发**: 评审完成后必须检查通知配置并尝试发送通知，不可跳过此步骤
