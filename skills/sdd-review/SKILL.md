...
- **手动模式**（`/sdd-review`）：交互式执行，展示评审摘要，等待用户决定是否迭代
- **Agent 模式**（由 `sdd-run` 自动调用）：3 个评审 Agent 并行执行，仲裁 Agent 合并结果，自动判定 PASS/ITERATE

两种模式共享以下所有领域规则。Agent 模式下各 Agent 的输出格式见 `agent-prompts/review-{a,b,c}.md` 和 `agent-prompts/arbitrate.md`。
...
> **v5 改进**：从"全 5 分 + 0 确认问题"调整为"≥ 4 + 无 HIGH 确认问题"。
> 旧标准让 R1 几乎必然 ITERATE，浪费评审开销。新标准在保证质量的前提下减少无效迭代。
...
- **功能完整性 < 4** → 功能缺失无法交付
- **需求一致性 < 4** → 实现偏离需求
- 存在 **HIGH 级确认问题**（≥2 Agent 共识）

**PASS 附带建议**（不阻塞交付）：
- MEDIUM/LOW 问题记录到 `summary.md` 的"后续优化建议"章节
- 仅 1 个 Agent 发现的 HIGH 问题降为"待确认 MEDIUM"，记录但不阻塞
...
当评审轮次 ≥ 2 时：
1. 评审 Agent 收到上一轮的 `fix-directives.md` 和修复涉及的文件列表
2. 评审范围缩小为：
   - 上一轮修复指令是否全部正确落实
   - 修复是否引入了新问题
   - 不需要重新审查未修改的文件
