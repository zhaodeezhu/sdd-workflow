...
| 阶段 | 文档 | 回答的问题 | 内容边界 |
|------|------|-----------|---------|
| Specify | spec.md | **What** — 做什么？为什么？ | 业务需求、用户故事、验收标准、功能分组与完成定义。禁止出现 SQL/代码/文件路径 |
| Test Cases | testcases.md | **Verify** — 怎么验证？ | 测试场景、Given-When-Then、测试金字塔、测试-实现映射 |
| Plan | plan.md | **How** — 怎么实现？ | 技术方案、架构设计、API 设计、数据模型、阶段合约 |
| Tasks | tasks.md | **When** — 什么时候做？ | 任务拆分、执行顺序、验收标准、并行标记 |
...
5. **可选通知配置**：引导配置飞书任务完成通知（也可后续通过 `/sdd-notify configure` 配置）
...
├── SDD 阶段合约与评审标准 ← 评审维度、权重、硬阈值、阶段合约
└── 经验教训          ← 项目积累的问题和解决方案（持续更新，Bug 修复自动沉淀）
```
...
│  │              sdd-run (全自动流水线编排器)                    │   │
│  │                                                          │   │
│  │   Phase 0    Phase A        Phase B   Phase C   Notify   │   │
│  │   需求澄清 → Planner     →  Generator → Evaluator → 通知  │   │
│  │              ┌──────────┐   ┌────────┐ ┌────────┐ ┌────┐ │   │
│  │              │ specify  │   │implement│ │ review │ │飞书│ │   │
│  │              │ testcase │   │(红-绿-  │ │(MACE   │ │卡片│ │   │
│  │              │ plan     │   │ 重构)   │ │ 3Agent)│ │推送│ │   │
│  │              │ tasks    │   └────────┘ └────────┘ └────┘ │   │
│  │              └──────────┘                                  │   │
│  └──────────────────────────────────────────────────────────┘   │
...
│  │  sdd-implement | sdd-review | sdd-notify                  │   │
│  └──────────────────────────────────────────────────────────┘   │
├─────────────────────────────────────────────────────────────────┤
...
| 模板 | 产出文档 | 核心章节 |
|------|---------|---------|
| spec-template.md | spec.md | 需求追溯、用户故事、功能需求、非功能需求、验收标准、功能分组与完成定义 |
| testcases-template.md | testcases.md | 测试策略、单元/集成/API/前端/E2E 测试、边界条件、测试-实现映射 |
| plan-template.md | plan.md | 架构设计、数据模型、API 设计、前后端实现、安全设计、阶段合约 |
| tasks-template.md | tasks.md | Phase 拆分、任务详情（文件/依赖/验收标准）、并行标记、工时估算 |
| constitution-template.md | constitution.md | 铁律、原则、架构约束、文档边界、阶段合约与评审标准、经验教训 |
| bug-report-template.md | BUG-xxx.md | Bug 描述、根因分析、修复方案、回归测试、经验沉淀 |
| spec-modular-template/ | spec/ (拆分) | 概述、用户故事、验收标准、约束、变更记录 |
| plan-modular-template/ | plan/ (拆分) | 架构、数据模型、后端API/实现、前端API/实现、安全、性能 |
...
- 界面需求（ASCII 示意）
- 边界条件和异常场景
- 功能分组与完成定义（SDD v2 新增，为 review 提供量化基准）
- 超过 500 行自动触发 `/sdd-split spec {id} --auto`
...
- API 设计（URL、参数、响应格式）
- 前后端实现设计
- 阶段合约（SDD v2 新增，每个 Phase 的交付物 + 验证标准 + 质量阈值）
- 超过 1000 行自动触发 `/sdd-split plan {id} --auto`
...
- 支持并行标记（标 [P] 的任务可并行执行）
- 工时估算（基于任务复杂度）
- 超过 800 行自动触发 `/sdd-split tasks {id} --auto`
