...
1. **文件必须保存**: 评审报告和总结**必须**使用 Write 工具保存为文件，不能只在终端展示。这是硬性要求，不可省略
2. **保存后确认**: 每次保存后必须用 Read 工具确认文件内容已完整写入
3. **独立性**: 每个评审 Agent 必须拥有独立上下文，不能看到 Generator 的思考过程
4. **并行性**: 3 个 Agent 必须在同一条消息中并行派发，不能串行执行
5. **上下文注入**: 文档内容直接注入 Agent prompt，确保 Agent 独立上下文完整
6. **具体性**: 每个问题必须关联到具体文档条款和代码位置
7. **可操作性**: 每个问题必须给出修复建议，不仅仅是指出问题
8. **建设性**: 评审不仅要发现问题，也要肯定亮点和值得复用的模式
9. **效率**: 评审应聚焦于合约项和关键质量标准，不做过度审查
10. **人类主权**: 评审结果由人类审批，AI 不自行决定通过或失败
11. **共识优先**: ≥2 个 Agent 确认的问题优先级高于单 Agent 发现的问题
12. **技术栈适配**: 审查标准应结合项目实际技术栈和架构模式（参考 constitution.md）
